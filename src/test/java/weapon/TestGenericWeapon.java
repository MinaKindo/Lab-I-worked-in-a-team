...
public class TestGenericWeapon {
   ...
   * @throws WeaponException If out of range
   */
  @Test
  ...
  /**
   * Tests to make sure you cannot do damage when out of ammo
   * Also tests to make sure ammo does not go into the negative
   * @throws WeaponException
   */
  @Ignore
  public void testOutOfAmmo() throws WeaponException {
    GenericWeapon weapon = new MockWeapon();
    while(weapon.getCurrentAmmo() > 0) {weapon.fire(10);}
    weapon.reload();
    assertEquals(weapon.getMaxAmmo(), weapon.getCurrentAmmo());
    while(weapon.getCurrentAmmo() > 0) {weapon.fire(10);}
    weapon.fire(10);
